...
resource "azurerm_linux_virtual_machine" "vm" {
  ...
  provisioner "remote-exec" {
    inline = [ "mkdir test && touch test/test.txt" ]
      connection {
        type = "ssh"
        user = var.overall.username
        password = var.overall.password
        host = azurerm_linux_virtual_machine.vm[count.index].public_ip_address # azurerm_public_ip.pip[count.index].ip_address , self.public_ip_address
      }
  }
  provisioner "file" {
    destination = "test/output.tf"
    source = "./modules/vm/output.tf"
    connection {
      type = "ssh"
        user = var.overall.username
        password = var.overall.password
  ...
  provisioner "local-exec" {
    command =  "echo ${self.name}: ${self.public_ip_address} >> public_ip.txt"
  }
}
...
resource "azurerm_windows_virtual_machine" "wvm" {
  ...
  computer_name                   = "${var.overall.name}"
  admin_username                  = var.overall.username
  admin_password                  = var.overall.password
